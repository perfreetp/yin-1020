...
const MapDispatchPage: React.FC = () => {
  ...
  const pendingOrders = useMemo(() => {
    let list = mockOrders.filter(o => o.status === 'pending')
  ...
  return (
    <View className={styles.page}>
      ...
      <View className={styles.mapArea}>
        ...
        {/* 机械位置（空闲） */}
        {mockMachines.filter(m => m.status === 'idle').slice(0, 4).map((m, idx) => {
              ...
              >
                <View className={styles.chipTop}>
                  <View className={styles.chipFarmer}>
                    ...
                    <Text style={{
                      fontSize: 22, padding: '2rpx 10rpx',
                ...
                <View className={styles.chipAddr}>📍 {order.village} · {order.address}</View>
              </View>
            ))
          )}
        </ScrollView>
      ...
      <View className={styles.recommendSection}>
        <View className={styles.recommendHeader}>
          ...
          {selectedOrder && (
            <Text className={styles.selectedHint}>已选：{selectedOrder.farmerName}·{selectedOrder.area}亩</Text>
          )}
        </View>

        {!selectedOrder ? (
          <View className={styles.emptyHint}>
            👆 点击上方订单卡片或地图上的红色标记选择待派工订单
          </View>
